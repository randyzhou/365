...
function index_industry_new(allow_select_industry_count, target_industry_names, target_industry_ids,industrydiv_style,codeids, index_industry_callback) {
	...
	$(hi).each(function (i, data) {
		listHtml.push("<li><div class=\"category categorylt\"><label><input "+window.style+" type=\"checkbox\" value=\""+data+"\" name=\""+hn[data]+"\" />"+hn[data]+"</label></div><div class=\"detail\" ><ul>");
		$(ht[data]).each(function(i,datas){
			listHtml.push("<li><div class=\"\"><div class=\"flo_lin flo_lin_lt\"><label><input value=\""+datas+"\" hyonevalue=\""+data+"\" name=\""+hn[datas]+"\"  type=\"checkbox\">"+hn[datas]+"</label></div></div></li>");
		});	
		listHtml.push("</ul></div><div class=\"clear\"></div></li>");
	});
	$(".a_prof_n").html(listHtml.join(""));
...
function index_ltjob_new(allow_select_ltjob_count, target_ltjob_names, target_ltjob_ids,ltjobdiv_style, codeids,index_industry_callback) {
	...
	var num = 0;//二级类个数确定一级类行高
	$(ji).each(function (i, data) {
		listHtml.push("<li><div class=\"category categorylt\"><label><input "+window.style+" type=\"checkbox\" value=\""+data+"\" name=\""+jn[data]+"\" />"+jn[data]+"</label></div><div class=\"detail\" ><ul>");
		$(jt[data]).each(function(i,datas){
			listHtml.push("<li><div class=\"\"><div class=\"flo_lin flo_lin_lt\"><label><input value=\""+datas+"\" jobonevalue=\""+data+"\" name=\""+jn[datas]+"\"  type=\"checkbox\">"+jn[datas]+"</label></div></div></li>");
		});	
		listHtml.push("</ul></div><div class=\"clear\"></div></li>");
	});
	$(".a_prof_n").html(listHtml.join(""));
		...
		$(ji).each(function (i, data) {
			for(var i=0;i<LtjobArrayAll.length;i++){
				if(LtjobArrayAll[i]==data){
					$("#ChooseLtjobLayer .ShowLtjob").append("<div class=\"job\" code=\"" +data+ "\" type=\"jobone\" title=\"" +jn[data]+ "\" ><span style=\"width: 70px;\">" + jn[data] + "</span><label class=\"job_lab \"></label></div>");
			...
			$(jt[data]).each(function(i,data2){
				for(var i=0;i<LtjobArrayAll.length;i++){
					if(LtjobArrayAll[i]==data2){
						$("#ChooseLtjobLayer .ShowLtjob").append("<div class=\"job\" jobonecode=\""+data+"\" code=\"" + data2 + "\" type=\"jobtwo\" title=\"" +jn[data2]+ "\" ><span style=\"width: 70px;\">" +jn[data2] + "</span><label class=\"job_lab \"></label></div>");
...
$(document).ready(function(){
	...
	$("#ChooseLtjobLayer .category :input").live("change",function(){
		var code = $(this).attr("value");
		if ($(this).attr("checked")) {
			$("#ChooseLtjobLayer .ShowLtjob div[jobonecode=\"" +code + "\"]").remove();
			if($("#ChooseLtjobLayer .ShowLtjob").find("div").length >=window.allow_select_ltjob_count) {
				...
				$("#ChooseLtjobLayer .ShowLtjob").append("<div class=\"job\" code=\"" + code + "\" type=\"jobone\" title=\"" + $(this).attr("name") + "\" ><span style=\"width: 70px;\">" + $(this).attr("name") + "</span><label class=\"job_lab \"></label></div>");
	...
	$("#ChooseLtjobLayer .detail :input").live("change", function () {
		if ($(this).attr("checked")) {
			if($("#ChooseLtjobLayer .ShowLtjob").find("div").length >=window.allow_select_ltjob_count) {
				...
				$("#ChooseLtjobLayer .ShowLtjob").append("<div class=\"job\" jobonecode=\"" + $(this).attr("jobonevalue") + "\" code=\"" + $(this).attr("value") + "\" type=\"jobtwo\" title=\"" + $(this).attr("name") + "\" ><span style=\"width: 70px;\">" + $(this).attr("name") + "</span><label class=\"job_lab \"></label></div>");
			}
		}else{
			$("#ChooseLtjobLayer .ShowLtjob div[code=\"" + $(this).attr("value") + "\"]").remove();
		}
