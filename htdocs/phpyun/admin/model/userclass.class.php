...
class userclass_controller extends adminCommon{
	...
	function up_action(){
		...
		if($_GET['id']){
			...
			$class1				= 	$categoryM	->	getUserClass($whereOne);
			$class2				=	$categoryM	->	getUserClassList($whereTwo);
			$this	->	yunset("id",$id);
			$this	->	yunset("class1",$class1);
			$this	->	yunset("class2",$class2);
		}
		$position				=	$categoryM	->	getUserClassList(array('keyid'=>'0'));
		$this	->	yunset("position",$position);
		$this	->	yuntpl(array('admin/admin_userclass'));
	}
	...
	function ajax_action(){
		...
		$categoryM	->	addUserClass($addData,$whereData);
		echo '1';
		die;
	}
}
