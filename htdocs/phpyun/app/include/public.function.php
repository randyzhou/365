    ...
    function getLocalCity(){
        ...
        if(false !== $ret) {
            ...
            foreach($site_domain as $key=>$value ){
                if ($value['three_cityid'] && strpos($value['cityname'],$local) !== false){//查找分站缓存中的该三级城市
                    $cityInfo = $value;
                    break;
                }else if($value['cityid'] && $value['cityid']==$cityid){//没有该三级城市分站时查找二级城市分站
                    $cityInfo = $value;
                    break;
                }else if($value['province'] && $value['province']==$provinceid){//没有二级城市，查找是否有该城市上级的一级城市分站
                    $cityInfo = $value;
                    break;
                }
            }
