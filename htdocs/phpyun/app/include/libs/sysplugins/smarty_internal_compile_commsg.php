...
class Smarty_Internal_Compile_Commsg extends Smarty_Internal_CompileBase{
    ...
    public function compile($args, $compiler, $parameter){
        ...
        $OutputStr='global $db,$db_config,$config;$paramer='.ArrayToString($_attr,true).';$List=array();
		//处理传入参数，并且构造分页参数
		...
		if(is_array('.$name.')){
            ...
            foreach('.$name.' as $k=>$v){
				if($v[\'isnm\']==\'1\'){
					...
					'.$name.'[$k][\'photo\']= checkpic("",$config[\'sy_member_icon\']);
					...
					if($msgSex[$v[\'uid\']]==1){
						...
						'.$name.'[$k][\'photo\']= checkpic("",$config[\'sy_member_iconv\']);
					}
