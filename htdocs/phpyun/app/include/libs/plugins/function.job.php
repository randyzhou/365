...
function smarty_function_job($paramer,&$smarty){
	...
	if($paramer[id]){
		$id = $paramer[id];
		$Info = $db->select_alls("company_job","company","a.`state`='1' and a.`r_status`='1' and a.`id`='$id' and b.`uid`=a.`uid`","a.*,b.*,a.name as comname,a.provinceid as prov,a.cityid as city,a.three_cityid as three_city");
		if(is_array($Info))
		{
			//处理类别字段
