...
class usermodel {
	...
	function check_login($username, $password, &$user) {
		...
		} elseif(!passCheck($password,$user['salt'],$user['password'])) {
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		...
		$password = passCheck($password,$salt);
	...
	function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '') {
		...
		if(!$ignoreoldpw && !passCheck($oldpw,$data['salt'],$data['password'])) {
		...
		$sqladd = $newpw ? "password='".passCheck($newpw,$data['salt'])."'" : '';
