...
class company extends common{
    ...
    function __construct($tpl, $db, $def = '', $model = 'index', $m = '')
    {
        ...
        if (!empty($this->spid) && in_array($_GET['c'], array('info','uppic','right','ad','binding','pay','paylog','jobpack','integral','child'))) {
    ...
    function public_action()
    {
        ...
        if (!empty($guweninfo)) {
            ...
            $reportwhere    =   array('p_uid' => $this->uid,'type'=>2, 'orderby' => array('inputtime, desc'));
    ...
    function company_satic()
    {
        ...
        if (!empty($this->spid)) {
            ...
            $statis     =   $statisM->vipOver($this->spid, 2);
        ...
        $this->yunset('statis', $statis);
        $this->yunset('todayStart', strtotime(date('Y-m-d')));
