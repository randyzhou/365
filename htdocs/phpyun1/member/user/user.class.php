...
class user extends common{
	...
	function __construct($tpl,$db,$def='',$model='index',$m='') {
		...
		if($this->config['user_resume_status']=='1'){
			...
			if(!in_array($_GET['c'],array('expect','log')) && !in_array($_GET['act'],array('logout','add'))){
	...
	{ 
	    $reusmeM  =  $this->MODEL('resume');
	    $resume   =  $reusmeM -> getResumeInfo(array('uid'=>$this->uid),array('logo'=>1));
		...
		}elseif(in_array($_GET['c'], array('commsg','finder','favorite','look_job','job','invite','atn','partapply','partcollect','comment'))){
