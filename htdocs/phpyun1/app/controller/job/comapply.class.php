...
class comapply_controller extends job_controller
{
    ...
    function index_action()
    {
        ...
        if ($viplist['vip_etime'] == 0 || ($viplist['vip_etime'] != 0 && $viplist['vip_etime'] >= strtotime('today'))) {
    ...
    function history_action(){
        ...
        if ($_POST['id'] && $this->usertype == 1) {
            ...
            // 是否浏览过此职位，有则更改查看时间，无则添加数据
            $lookJobNum     =   $JobM -> getLookJobNum(array('uid' => $this->uid, 'jobid' => $id));
            
            if ($lookJobNum > 0) {
                
                $JobM -> updLookJob(array('uid' => $this->uid, 'jobid' => $id), array('datetime' => $time, 'status' => 0));
            } else {
                
                $JobInfo    =   $JobM -> getInfo(array('id' => $id), array('field' => '`name`, `uid`'));
                
                $JobM -> addLookJob(array('uid' => $this->uid, 'jobid' => $id, 'com_id' => $JobInfo['uid'], 'datetime' => $time,'did' => $this->userdid));
                
                $historyM   =   $this->MODEL('history');
                $historyM   ->  addHistory('lookjob', $id);
                
                $sysmsgM    =   $this -> MODEL('sysmsg');
                $resumeM    =   $this -> MODEL('resume');
                
                $resumeinfo =   $resumeM -> getExpect(array('uid' => $this->uid, 'defaults' => 1, 'r_status' => 1), array('field' => '`id`,`uname`' ));
                
                $msgS       =   '用户<a href="resumetpl,' . $resumeinfo['id'] . '">' . $resumeinfo['uname'] . '</a>浏览了您的职位' . $JobInfo['name'];
                
                $sysmsgM -> addInfo(array('uid' => $JobInfo['uid'],'usertype' => 2,'content' => $msgS));
            }
