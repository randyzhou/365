...
class ajax_controller extends common{
	...
	function mobliecert_action(){
	    ...
	    if(!$this->uid || !$this->username){
	        ...
	        $result 		= $noticeM->sendCode($moblie, 'cert', $user);
	...
	function atn_teacher_action(){//关注讲师
		$data	=	array(
			'id'			=>	(int)$_POST['id'],
			'tid'			=>	(int)$_POST['tid'],
			'uid'			=>	$this->uid,
    ...
    function changeusertype_action(){
      ...
      $res		=	$memberM -> changeUsertype(array('uid'=>$this -> uid,'usertype'	=>	$changeType,'spid'=>$this -> spid));
      
      if($res['errcode'] == 9){

        $user	=	$res['memInfo'];
        
        $this->cookie->add_cookie($this -> uid, $this -> username, $user['salt'], $user['email'], $user['password'], $changeType, 1, $user['did']);
        
        
      }
      echo json_encode(array('errcode'=>$res['errcode'],'msg'=>$res['msg']));
    }
    // 切换账号
