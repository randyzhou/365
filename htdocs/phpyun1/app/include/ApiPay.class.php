		...
		if($order['order_state']=='1' && $order['id']){
			...
			if($type == 1 && $ratingId && $usertype != 1){
				...
				if($order_info['eid']){
				    ...
				    $expect =   $resumeM -> getExpect(array('id' => $eid), array('field' => '`id`,`uid`,`uname`,`height_status`'));
			    ...
			    if($order['once_id']){
 			    ...
 			    if($order['order_id']){
 			        ...
 			        $adM    =   $this -> MODEL('ad');
 			        $ad     =   $adM  ->   getAdOrderInfo(array('order_id' => $orderid), array('field' => '`price`'));
					if(!empty($ad)){
                    ...
                    if (empty($zphCom)) {
                        ...
                        if ($nid) {
                            ...
                            if ($order_info['zph_integral'] && $order['integral']) { // 充值积分报名招聘会
                                ...
                                $integralM -> insert_company_pay($order_info['zph__integral'],2,$uid,$order['usertype'],"预定招聘会，扣除".$this->config['integral_pricename'],1,2,false);
			 ...
			 if($nid){
				...
				$orderM -> upInfo($order['id'], array('order_state' => '2', 'order_type' => $paytype));
