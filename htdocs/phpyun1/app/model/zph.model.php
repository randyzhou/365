	...
	*/
	public function getZphCompanyList($whereData , $data=array()){
		...
		if($List && is_array($List)){
			...
			$listA	  =  $this -> select_all('company_job',array('uid'=>array('in',pylode(',',$uid)),'state' => 1, 'status' => 0,'r_status' => 1),'`id`,`uid`,`name`');
	...
	public function getZphSpaceInfo($whereData , $data = array('field'=>null)){
        ...
        if (!empty($Info)) {
            ...
            if ($Info['content']) {
                ...
                $subject            =   strip_tags($Info['content']); 
                $pattern            =   '/\s/'; 
	            ...
	            if (strtotime($zph['starttime']) < time()) {
	                ...
	                if ($comInfo['r_status'] != 1) {
						...
						if (!empty($zphcom)) {
							...
							$price  =   floatval($space['price'] / $this->config['integral_proportion']); 
							
							if ($statis['vip_etime'] >= strtotime('today') || $statis['vip_etime'] == 0) {
								
								if ($statis['rating_type'] == 1) {
									
									if ($price == 0 && $statis['zph_num'] == 0) { 
