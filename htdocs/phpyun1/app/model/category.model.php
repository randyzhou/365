...
class category_model extends model{
	...
	function delUserClass($whereData=array(),$data=array()){
		...
		$ids				=	$this	->	sonclass('userclass',$id);
		if(count($ids)){
	...
	function delCityClass($whereData=array(),$data=array()){
		...
		$ids			=	$this	->	sonclass('city_class',$id_arr);
		if(count($ids)){
	...
	function getJobClassList($whereData=array(), $field = '*',$data=array()){
		if(!count($data)){
			...
			if($data['type']=='oneall'){
				$twojob		=	array();
				$threejob	=	array();
				$twojob		=	$this	->	select_all('job_class',$whereData,'id,sort,name,e_name');
				if(is_array($twojob)){
					$val	=	array();
	...
	function delJobClass($whereData=array(),$data=array()){
		...
		$ids				=	$this	->	sonclass('job_class',$id);
		if(count($ids)){
	...
	function delPartClass($whereData=array(),$data=array()){
		...
		$ids				=	$this	->	sonclass('partclass',$id);
		if(count($ids)){
	 ...
	 * $field		自定义查询字段
	 */
	function getCrmClassList($whereData=array(),$field='*'){
	    $list	=	array();
	    $list	=	$this	->	select_all('crm_client_class',$whereData,$field);
	    return	$list;
	}
