...
class lookresume_model extends model{
	...
	private function getDataList($List = array(), $data = array()){
	    ...
	    $reData['field']   =   '`id`,`uid`,`name`,`job_classid`,`minsalary`,`maxsalary`,`height_status`,`exp`,`city_classid`,`hy`,`lastupdate`,`edu`,`birthday`,`sex`,`uname`';
	    $expect	           =   $this -> getResumeExpectList($reWhere, $reData);
	    if (!empty($expect['list']) && is_array($expect['list'])) {
	        foreach ($expect['list'] as $ev) {
	            $userid[$ev['uid']]    =   $ev['uid']; 
	        }
	    }
        ...
        if ($usertype !=2) {
	       ...
	       $com    =	$this -> select_all('company',array('uid'=>array('in',pylode(',', $comid))),'`uid`,`name`,`pr`,`mun`,`sdate`');
        }
	    if ($usertype == 2) {
