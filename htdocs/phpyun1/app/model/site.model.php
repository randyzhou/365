		...
		if(!empty($data)){
			...
			if(is_array($rids)&&$rids) {
				...
				$Table	=	array(
					...
					'company_msg','look_job','member_statis',
			...
			if(is_array($comids)&&$comids) {
				...
				$Table	=	array(
					...
					'company_product','partjob','hotjob','user_change','company_account'
				);
				$this -> updDid(array('userid_msg'), array('fid' => array('in', $uid)), $didData);
				
				$this -> updDid(array('ad_order'),array('comid'=>array('in', $uid)),$didData);
			}
			
			
			if(is_array($rids)||is_array($comids)||is_array($ltids)) {
				
				$this -> updDid(array('report'), array('p_uid' => array('in', $uid)), $didData);
				
			}
			if(is_array($comids)||is_array($ltids)) {
