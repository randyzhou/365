...
class weixin_model extends model{
		...
		if($userBind['bindtype']=='1')
		{
			...
			if($jobNum>0){
				...
				if($membeStatis['rating_type']=='2'){
					if($membeStatis['vip_etime']>=strtotime('today')){//有效期之内
					...
					if(($membeStatis['vip_etime']>=strtotime('today')) && $membeStatis['breakjob_num']>=$jobNum){
						$this->update_once('company_job', array('lastupdate' => time()), array('uid' => $userBind['uid'], 'state' => 1, 'status' => 0 , 'r_status' => 1));
	...
	{	
	    ...
	    if($wxid){
	        ...
	        if (empty($User['unionid']) && !empty($unionid)){  //原先只绑定公众号，没绑定开放平台
	            $this->update_once('mmeber', array('unionid' => $unionid), array('uid' => $User['uid']));
	...
	function valid($echoStr,$signature,$timestamp,$nonce)
  {
				...
				if(is_array($usertList) && !empty($usertList))
				{
					...
					foreach($usertList as $value){
						...
						if($Expect['maxsalary']>0){
							$Sarlary	= '￥'.$Expect['minsalary'].'-'.$Expect['maxsalary'].'元/月';
						}elseif($Expect['minsalary']>0){
							$Sarlary	= '￥'.$Expect['minsalary'].'以上/月';
						}else{
							$Sarlary	= '面议';
						}
	...
	function sendWxresume($data){
	   ...
	   global $config;
		...
		if($data['uid'])
		{
			...
			//提取绑定微信的用户
			$userInfo    =   $this->select_once('member', array('uid' => $data['uid']), '`username`,`wxid`');
	...
	function sendWxAdmin($data=array()){
		...
		if($config['wx_xxtz']!='1' || !$config['wx_mbadmin'])
		{
			return true;
		}
		...
		if($data['wxid'])
		{
			...
			$this->sendWxTemplate($data['wxid'],$Tempid,'',$TempDate);
		}
	}
	...
	function sendSubscribe($data = array()){
		...
		//获取用户的微信id
		$userInfo		=	$this -> select_once('member', array('uid' => $data['uid']), '`wxid`');
		...
		$TempDate['keyword2']	=	array('value' => '订阅已发送', 'color'=>'');
		$TempDate['remark']		=	array('value' => $Remark, 'color'=>'');
		$Url					=	Url('wap',array('c'=>'login'));
	...
	function isWxlogin($wxid,$wxloginid){
		...
		if($wxqrcode['auid']>0){
			...
			if($userBind['uid']>0){//已绑定微信账户 直接登录
				//更改缓存状态
				...
				//    return array('result'=>true,'type'=>'binded');
				//}
	...
	*/
    ...
    function sendWxChat($data){
        ...
        if($data['wxid'])
        {
            ...
            $TempDate['remark']	    = array('value'=>$Remark,'color'=>'');
            $Url = Url('wap',array('c'=>'chat','a'=>'chatList'));
	...
	function sendWxUsercahnge($data){
		...
		if($data['uid'] && $data['id'] ){
			...
			$Member = $this->select_once('member', array('uid' => $user['uid']), '`wxid`,`uid`');
			...
			$Url = Url('wap',array('c'=>'login'));
			$this->sendWxTemplate($Member['wxid'],$Tempid,$Url,$TempDate);
		}
	}
}
?>