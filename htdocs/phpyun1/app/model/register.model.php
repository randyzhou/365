...
class register_model extends model{
	...
	function regMoblie($post=array()){
	    if($post['moblie'] || $post['email']){
			if($this->config['sy_web_mobile']!="" && $post['moblie']){
		...
		if(!passCheck($data['pw'],$user['salt'],$user['password'])){
			...
			if($data['mobile']!=""){
				...
				if($user['moblie'] == $data['mobile']){
					...
					$this->update_once("member",array('moblie'=>''),array('uid'=>$data['zyuid']));
				}
				
				if($user['usertype']=='1'){
					
					$nid 	= 	$this->update_once("resume",array('telphone'=>'','moblie_status'=>0),array('uid'=>$data['zyuid']));
					
				}else if($user['usertype']=='2'){
					
					$nid 	= 	$this->update_once("company",array('linktel'=>'','moblie_status'=>0),array('uid'=>$data['zyuid']));
				...
				if($user['email'] == $data['email']){
					...
					$this->update_once("member",array('email'=>''),array('uid'=>$data['zyuid']));
					
				}
				if($user['usertype']=='1'){
					
					$nid 	= 	$this->update_once("resume",array('email'=>'','email_status'=>0),array('uid'=>$data['zyuid']));
					
				}else if($user['usertype']=='2'){
					
					$nid 	= 	$this->update_once("company",array('linkmail'=>'','email_status'=>0),array('uid'=>$data['zyuid']));
