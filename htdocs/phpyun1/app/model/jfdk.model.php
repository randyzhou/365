	...
	function dkBuy($data = array())
    {
        if ($data['uid']) {
            ...
            if ($data['logid']) {
                ...
                $this->update_once('user_log', array('status' => $status), array('id' => $data['logid']));
            }
        }else{
            $return = array(
                'error' => 1,
			...
			if($data['ratingid']){
				...
				if(empty($ratinginfo)){
					...
					if($statis['integral'] >= $integral_dk){
						...
						$this	->	addMemberLog($uid,$usertype,$ratinginfo['name'].'购买成功');
			    ...
			    if ($zphcom && is_array($zphcom)) {
    			    ...
    			    if($statis['integral'] >= $dkjf){
    			        ...
    			        if($status){
    			            ...
    			            $integralM -> company_invtal($uid, $usertype,$dkjf, false,$this->config['integral_pricename'].'抵扣预定招聘会',true,2,'integral');
    			            
    			            $this->addMemberLog($uid, $usertype,'报名招聘会,ID:'.$data['zid'].',展位：'.$bid,14,1);
    			            
    			            require_once('admin.model.php');
    			            $adminM  =   new admin_model($this->db,$this->def);
    			            $adminM -> sendAdminMsg(array('first'=>'有新的招聘会报名需要审核','type'=>0));
