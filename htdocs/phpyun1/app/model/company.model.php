...
class company_model extends model{
    ...
    public function getInfo($uid, $data = array('field'=>'','utype'=>'','logo'=>'')) {
        ...
        if (!empty($uid)) {
            ...
            if($ComInfo && is_array($ComInfo)){
                ...
                if($ComInfo['shortname'] && !empty($data['short'])){
                ...
                if($data['utype']=='wxapp'){
					...
					$ComInfo['address_n']     =   mb_substr($ComInfo['address'],0,18,"utf-8");
                }else{
				...
				if (!empty($data['edit'])) {
                    ...
                    if($ComInfo['linkphone']){
                        ...
                        $linkphone                =  @explode('-',$ComInfo['linkphone']);
                        $ComInfo['phoneone']      =  $linkphone[0];
                        $ComInfo['phonetwo']      =  $linkphone[1];
                        $ComInfo['phonethree']    =  $linkphone[2];
    ...
    public function upInfo( $id , $where = array() , $Data = array()) {
        ...
        if (!empty($id) || $where) {
            ...
            $where  =   !empty($where) ? array_merge($where, $whereD) : $whereD;
    ...
    private function upCertMobile($whereData = array(), $upData = array(), $data = array()){
        ...
        if ($cert && is_array($cert)) {
            ...
            $codeTime   =   $cert['ctime'] + $this->config['moblie_codetime'] * 60 ; 
            ...
            if ($codeTime < $time) { 
                ...
                $this -> update_once('resume', array('moblie_status'=>'0', 'telphone'=>''), array('telphone'=>$moblie, 'uid'=>array('<>',$uid)));
    ...
    */
    public function setBanner($data  =   array()){
        if(!empty($data)){
            ...
            if(isset($picurl)){
                $cominfo	=	$this -> select_once('company',array('uid'=>$data['uid']),array('field'=>'r_status'));
                $valueData  =   array(
                    'pic'       =>  $picurl,
            ...
            if($data['type']=='add'){
                ...
                if($row>0){
                    ...
                    $this -> addMemberLog($data['uid'],$data['usertype'],"上传企业横幅",16,1);
                ...
                $this -> addMemberLog($data['uid'],$data['usertype'],"修改企业横幅",16,2);
                ...
                if ($return['id']){
                    ...
                    if ($data['utype'] == 'user'){
                        ...
                        if(!empty($com)  && $com['lastupdate'] == ''){
							...
							if ($this->config['sy_crm_duty'] == '2' && $com['crm_uid'] == '' && $com['crm_time'] == '') { 
     ...
     */
    private function setStatisInfo($uid, $sData){
        ...
        if (!empty($sData['rating'])) {
            ...
            $comRating  =  $this -> select_once('company_statis',array('uid' => $uid),'`rating`');
            ...
            if ($sData['rating'] != $comRating['rating']){
                ...
                $statisM->upInfo($rvalue, array('uid' => $uid, 'usertype' => 2, 'adminedit' => '1', 'info' => $rinfo));
            } else {
                
                 $this->update_once('company_statis', $sData, array('uid' => $uid));
    ...
    private function setJobInfo($uid, $comData)
    {
        ...
        if (! empty($job)) {
            ...
            if ($job['pr'] != $comData['pr']) {

                $cjdata['pr'] = $comData['pr'];
                
            }
			if ($job['num'] != $comData['mun']){
                
                $cjdata['mun']             =  $comData['mun'];
                
            }
			if ($job['com_provinceid'] != $comData['provinceid']){
                
                $cjdata['com_provinceid']  =  $comData['com_provinceid'];
                
            }
			if ($job['rating'] != $comData['rating']){
                
                $cjdata['rating']          =  $comData['rating'];
                
            }
			if ($job['r_status'] != $comData['r_status']){
            ...
            $welfare  =  @explode(',', $comData['welfare']);
            $oldwel   =  @explode(',', $job['welfare']);
            $welDiff  = array_diff($welfare, $oldwel);
            
            if (!empty($welDiff)){
                
                $cjdata['welfare']  =  $comData['welfare'];
            }
        }
        
        if (!empty($comData['name']) && $comData['name'] != $job['com_name']){
    ...
    private function setFieldCheck($uid, $comData, $data = array('wap' => null)){
        ...
        if (! $data['wap']) {
            ...
            if ($comData['phonetwo'] && $comData['phonetwo'] != '座机号') {

                if ($comData['phoneone'] == '' || $comData['phoneone'] == '区号') {

                    $return['msg']	=  '请填写区号！';
                }else{
                    $linkphone[]	=  $comData['phoneone'];
                }
                
                if($comData['phonetwo']&&$comData['phonetwo']!='座机号'){
                    $linkphone[]	=  $comData['phonetwo'];
                }
                
                if($comData['phonethree']&&$comData['phonetwo']!='分机号'){
                    $linkphone[]	=  $comData['phonethree'];
                }
                $comData['linkphone']  =  pylode('-',$linkphone);
            }
        }
