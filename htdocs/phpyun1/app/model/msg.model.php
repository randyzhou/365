...
class msg_model extends model{
	 ...
	 * 应用中使用到的添加求职咨询
	 */
	public function addMsg($data = array(),$code=array()){
		$res                =   array();
		$res['errorcode']   =   8;
		$res['msg']         =   '';
		...
		$data['job_uid']	=	intval($data['job_uid']);
		...
		$resume					=	$this->select_once("resume",array('uid'=>$data['uid']),"`name`");
		
		if(!empty($resume)){
			
			$data['username']   =   $resume['name'];
		...
		$data['type']         	=   1;
		$data['datetime']     	=   time();
		$id						=	$this -> addInfo($data);

		if(!empty($id)){
			include_once ('sysmsg.model.php');
			$sysmsgM    		=   new sysmsg_model($this->db, $this->def);
			$sysmsgM -> addInfo(array('uid' => $data['job_uid'],'usertype'=>2,  'content' => '您有一个留言，请及时查看'));
	...
	public function delInfo($id,$data=array()){
	    ...
	    if (!empty($id)){
	        ...
	        $nid      =  $this->delete_all('msg',array('id'=>array('in',$id)),$limit);
