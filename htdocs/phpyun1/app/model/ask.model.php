	...
	public function addAskInfo($data = array()){
		if(!empty($data)){
			...
			if($this->config['integral_question_type']=="1"){
				...
				$statis		=	$statisM -> getInfo($data['uid'],array("field"=>"`integral`","usertype"=>$data['usertype']));
				
				if($statis['integral']<$this->config['integral_question']){
					
					$return['msg']	=	"您的！".$this->config['integral_pricename']."不足，请先充值后再发布问题！";
				}
				$auto=false;
			} 
			...
			if($data['content']&&$data['id']){
				...
				if($this->config['integral_answer_type']=="1"){
					...
					if($statis['integral']<$this->config['integral_answer']){
						$arr['msg']		=	"您的！".$this->config['integral_pricename']."不足，请先充值后再回答问题！";
	...
	public function addReview($addData = array()){
		...
		if(!empty($addData)) {
			...
			if($this->config['integral_answerpl_type']=="1"){
				...
				$statis		=	$statisM -> getInfo($addData['uid'],array("field"=>"`integral`","usertype"=>$addData['usertype']));
				if($statis['integral']<$this->config['integral_answerpl']){
					return "您的".$this->config['integral_pricename']."不足，请先充值后再评论问答！";die;
				}
				$auto=false;
			}
	...
	function setAttention($data = array()){
		...
		if($info['uid']==$data['uid']){
			...
			if($ids[0]==''||$ids[0]=='0'){
				...
				$this -> addMemberLog($data['uid'],$data['usertype'],$log,5,1);
				...
				$this -> addMemberLog($data['uid'],$data['usertype'],$n_log,5,1);
