...
class resume_model extends model{
	...
	*/
	...
	private function setExpectInfo($uid, $rData)
	{
	    ...
	    if (!empty($expect)){
	        ...
	        }elseif (!empty($rData['birthday']) &&  $expect['birthday'] != $rData['birthday']){
	            
	            $eData['birthday']  =  $rData['birthday'];
	            
	        }elseif (!empty($rData['exp']) && $expect['exp'] != $rData['exp']){
	            
	            $eData['exp']       =  $rData['exp'];
	            
	        }elseif (!empty($rData['edu']) && $expect['edu'] != $rData['edu']){
	            
	            $eData['edu']       =  $rData['edu'];
	            
	        }elseif (!empty($rData['name']) && $expect['uname'] != $rData['name']){
	            
	            $eData['uname']     =  $rData['name'];
	            
	        }elseif (!empty($rData['r_status']) && $expect['r_status'] != $rData['r_status']){
	...
	public function getInfo($data = array('uid'=>null,'eid'=>null,'needCache'=>null,'tb'=>null))
	{
	    ...
	    if ($resume['uid'] && !empty($data['eid'])){
	        ...
	        if ($expect['id'] && $data['tb'] == 'all'){
	            ...
	            $training  =    $this -> getResumeTrains(array('eid'=>$eid,'orderby'=>'sdate,desc'));
	...
	public function getInfoByEid($data = array())
	{
		...
		if(in_array($usertype, array(2, 3))){
			...
			if(!empty($row)){
				...
				if($this->config['com_lietou_job'] == 1 && ($row['vip_etime'] >= strtotime('today') ||  $row['vip_etime'] == 0)){
				...
				if($row['down_resume'] > 0 && intval($listNum) > 0){
					...
					$resume['downresumes']		=	$row['down_resume']; 
				}
			}
		}
	...
	public function getExpect($whereData,$data = array())
	{
	    ...
	    if ($expect && !empty($data['needCache'])){
	        ...
	        if($expect['minsalary'] && $expect['maxsalary']){
	            ...
	            $expect['salary']  =   $expect['minsalary'].'-'.$expect['maxsalary'].'元/月';
	        }elseif ($expect['minsalary']){
	            
	            $expect['salary']  =   $expect['minsalary'].'元以上/月';
	        }else{
  ...
  /**
     ...
     * $whereData       查询条件
      * $data            自定义处理数组
    * 简单完整度的查询返回，复杂的，访问getUserResumeInfo方法
     */
   ...
   /**
	...
	public function delResumeOut($id,$data=array()){
	    ...
	    if (!empty($id)){
	        ...
	        $return['id']  =  $this -> delete_all('resumeout',array('id'=>array('in',$id)),$limit);
	...
	function setDayprice($eid,$data=array()){
		...
		if(!$rdayprice){
			if($data['integral']&&$data['integral']=='yes'){
				...
				$newdayprice	=	$this->config['integral_down_resume'] * $this->config['integral_proportion'];
			}else{
			
				$newdayprice	=	$this->config['integral_down_resume']; 
			}
		}else{
    ...
    public function topResumeCheck($data = array('eid'=>null,'uid'=>null)){
        ...
        $expect   =  $this->select_once('resume_expect',array('id'=>$data['eid'],'uid'=>$data['uid']),'`doc`');
        ...
        if($data['id']){
            ...
            if(is_array($resume)){
                ...
                foreach($job as $k=>$v){
                    ...
                    if(in_array($v['job_city_three'],$city_classname)){
                        $pre		=	$pre+10;
                    }
                    if($resume['useredu']==$v['job_edu'] || $v['job_edu']=="不限"){
                        $pre		=	$pre+5;
                    }
                    if($resume['user_marriage']==$v['job_marriage'] || $v['job_marriage']=="不限"){
                        $pre		=	$pre+5;
                    }
                    if($resume['sex']==$v['job_sex'] || $v['job_sex']=='不限'){
                        $pre		=	$pre+5;
                    }
                    if($resume['report']==$v['job_report'] || $v['job_report']=="不限"){
                        $pre		=	$pre+5;
                    }
                    if($resume['user_exp']==$v['job_exp'] || $v['job_exp']=="不限"){
                        $pre		=	$pre+5;
                    }
                    $job[$k]['pre']	=	$pre;
					if(empty($v['job_sex'])){
    ...
    **/
    public function addResumePage($data=array('uid'=>null,'needcache'=>null,'from'=>'pc')){
    	...
    	if($data['uid']){
			...
			$error	=	$this->addResumeRequireCheck($data);
			$return['error']	   =	$error;
	...
	function setUserCrm($data = array(), $whereData = array()) {
	    ...
	    if(!empty($data) && !empty($whereData)){
	        ...
	        $return['id']   =   $this -> update_once('resume', $data, array('uid' => array('in', $uids)));
