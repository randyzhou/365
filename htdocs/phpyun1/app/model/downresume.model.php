...
class downresume_model extends model{
	...
	private function getDataList($List,$data = array()){
        ...
        $reData['field']  =  '`id`,`name`,`job_classid`,`minsalary`,`maxsalary`,`height_status`,`exp`,`hy`,`lastupdate`,`city_classid`,`edu`,`sex`,`birthday`';
	    ...
	    foreach($List as $k=>$v){
	        $List[$k]['status']	=	$cache['comclass_name'][$v['status']];
	...
	public function downResume($data = array()){
		...
		if ($usertype == 1){  
		    ...
		    if($this->config['com_lietou_job'] == 1 && ($statis['vip_etime'] >= strtotime('today') ||  $statis['vip_etime'] == 0)){ 
