		...
		if(!empty( $List )){
			...
			if($job&&is_array($job)){
				foreach($job as $k=>$v){
					...
					if($v['minsalary']&&$v['maxsalary']){
						$v['job_salary'] =$v['minsalary']."-".$v['maxsalary']."元/月";
					}elseif($v['minsalary']){
						$v['job_salary'] =$v['minsalary']."以上元/月";
					}else{
						$v['job_salary'] ="面议";
					}
					$jobinfo[$v['uid']][]=$v;
			...
			foreach($List as $key=>$val){
				...
				if ($data['utype']=='wxapp') {
					foreach($company['list'] as $v){
						if($val['uid']==$v['uid']){
							$List[$key]['name_n']	=	mb_substr($v['name'],0,6,"utf-8");
							$List[$key]['hyname']	=	$cache['industry_name'][$v['hy']];
