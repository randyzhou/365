...
class once_model extends model{
   ...
   /**
    ...
    public function addOnceInfo($data = array()){
		...
		if($data['type']!='admin'&&$data['type']!='wxapp'){
		...
		if(!empty($id)){
			...
			if($nid){			
				if($this->config['com_fast_status']=="0" && $data['type'] !='admin'){
			...
			if($this->config['once_pay_price'] != "0" && $this->config['once_pay_price'] != "" && $data['type'] != 'admin'){
				$post['pay']        =   '1';
			}else{
				$post['pay']        =   '2';
			}
   ...
   /**
	...
	public  function setOnceCtime($id,$data = array()){
		...
		if(is_array($ids)){
			...
			if($return['id']){
				$return['msg']		=	'店铺招聘延期(ID:'.$jobid.')设置成功！';
				$return['errcode']	=	9;
			}else{
				$return['msg']		=	'店铺招聘延期(ID:'.$jobid.')设置失败！';
				$return['errcode']	=	8;
			}
		}else{
