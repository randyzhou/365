...
class statis_model extends model{
	...
	function getInfo($uid, $data = array())
    {
        if (! empty($uid)) {
            ...
            if ($statis && is_array($statis)) {
                ...
                $statis['vip_etime_n'] =   date('Y-m-d', $statis['vip_etime']);
                return $statis;
            }
        }
    }
    ...
    public function vipOver($uid, $usertype = null){
        ...
        if($usertype==2){
            ...
            if($statis['vip_etime']<$endtimevip && $statis['vip_etime']>0){
                $statis['remind']=1;
            }
        }
       
        if($statis['vip_etime'] < strtotime('tomorrow')){ 
            ...
            if($nums == 0){
                ...
                if($this->config['com_vip_done'] == '0'){
                    ...
                    $this -> update_once('company_job', array('rating' => 0, 'status' => 1), array('uid' => $uid));
    ...
    function getCom($data=array())
    {
        ...
        if ($statis['rating_type'] && $statis['rating']) {
            ...
            if ($statis['rating_type'] == '1' && $statis['job_num'] > 0 && ($statis['vip_etime'] == '0' || $statis['vip_etime'] >= strtotime('today'))) {
                
                $arr    =   array(
                    'job_num' => array('-', 1)
                );
                
            } elseif ($statis['rating_type'] == '2' && ($statis['vip_etime'] >= strtotime('today') || $statis['vip_etime'] == '0')) {
