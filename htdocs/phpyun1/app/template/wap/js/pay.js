...
function integral_add_buy() {
	...
	if(parseInt(integral) < parseInt(integral_need)) {
		layermsg(pricename+"不足，请先充值", 2, function() {
...
function integral_form() {
	...
	if(parseInt(min_integral) > parseInt(integral)) {
		layermsg('最低充值：' + min_integral + pricename, 2);
		return false;
	}
...
function integral_server_buy(type, id, bid) {
	
	var id   	= 	id;
	var ajaxUrl	= 	'index.php?c=dkzf';
	var payUrl	= 	'index.php?c=pay';
	
	var days 	= 	$('#days').val();
	
	if(type == 'autojob') {
		
		var rurl  = 'index.php?c=job';
		var pData = {jobautoids: id, rdays: days};
		
	}else if(type == 'zdjob'){

		var rurl  =	'index.php?c=job';
		var pData = {zdjobid: id, xsdays: days};
		
	}else if(type == 'recjob') {
		
		var rurl  =	'index.php?c=job';
		var pData = {recjobid: id, recdays: days};
		
	} else if(type == 'ujob') {
		
		var rurl  =	'index.php?c=job';
		var pData = {ujobid: id, udays: days};
 		 
	} else if(type == 'sxjob') {
		
		var rurl  =	'index.php?c=job';
		var pData = {sxjobid: id};
		 
	} else if(type == 'sxpart') {
		
		var rurl  =	'index.php?c=part';
		var pData = {sxjobid: id};
		
	} else if(type == 'sxltjob') {
		
		var rurl  =	'index.php?c=lt_job';
		var pData = {sxltjobid: id};
		
	} else if(type == 'downresume') {
		
		var rurl  =	wapurl + '/index.php?c=resume&a=show&id=' + id;
		var pData = {eid: id};
		
	} else if(type == 'issuejob') {
		
		var rurl  =	'index.php?c=jobcolumn';
		var pData = {issuejob: 1};
		 
	} else if(type == 'invite') {
		
		var rurl  =	wapurl + '/index.php?c=resume&a=invite&uid=' + id;
		var pData = {invite: 1};
		
	} else if(type == 'createson') {
		
		var rurl  =	'index.php?c=child';
		var pData = {createson: 1};
		 
	}else if(type='zph'){
		
		var rurl  =	wapurl + 'index.php?c=zph&a=reserve&id='+id;
		var pData = {zid: id, bid: bid};
	}
