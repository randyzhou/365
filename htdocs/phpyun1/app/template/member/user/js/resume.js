...
//	var three_cityid = $.trim($("#three_cityid").val());
	...
	$.post("index.php?c=expect&act=saveexpect", {name:name, hy:hy, job_classid: job_classid, city_classid: city_classid, minsalary: minsalary, maxsalary: maxsalary, jobstatus: jobstatus, type: type, report: report, eid: eid, submit: "1", dom_sort: getDomSort() }, function (data) {
		...
		if(data==0){
			...
			$("#saveexpect").hide();
			$("#eid").val(data.id);
			var html='<li>期望岗位：'+data.name+'</li><li>工作职能：'+data.job_classname+'</li><li>期望工作地点：'+data.city_classname+'</li><li>期望从事行业：'+data.hy_n+'</li><li>期望薪资：'+data.salary+'</li><li>到岗时间：'+data.report_n+'</li><li>期望工作性质：'+data.type_n+'</li><li>求职状态：'+data.jobstatus_n+'</li>';
			$("#expect").html(html);
...
function resumeFanhui(frame_id){ 
	...
	var specnum = $(window.frames[frame_id].document).find("#specnum").val(); //显示教育经历专业为空的
	if(specnum !=null && wrong !=null){
		var specnums=specnum.split('-');
		for(var i=0;i<specnums.length;i++){
			$("#m"+wrong+specnums[i]).show();
		}
	}
