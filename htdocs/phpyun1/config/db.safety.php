...
function gpc2sql($str,$str2) {
	if(preg_match("/select|insert|delete|load_file|outfile/is", $str)){
		exit(safe_pape());
	}
	if(preg_match("/select|insert|delete|load_file|outfile/is", $str2)){
		exit(safe_pape());
	}
...
function common_htmlspecialchars($key,$str,$str2,$config){	
	if(is_array($str)){
		...
		//非编辑器字段内容 去除所有标签并进行SQL注入检查
		if(!in_array($key,array('content','config','group_power','description','body','job_desc','eligible','other','code','intro','doc','traffic','media','packages','booth','participate','expinfo','eduinfo','skillinfo','projectinfo'))){
			//$str = str_replace(" ","",$str);
			$str = strip_tags($str);
...
if(!empty($_POST)){
	...
	if($_SESSION['xsstooken']!=sha1($config['sy_safekey']) || empty($config['sy_safekey']))
	{
		
		foreach($_POST  as $id=>$v){
		    if(!in_array($id,array('preview','uimage','safekey','notify_data','token','deviceToken','xcxCode','rsaSign','sy_publicKey','sy_privateKey','sign','fund_bill_list','buyer_logon_id'))){
		...
		foreach($_POST  as $id=>$v){
			if (preg_match('/^[_0-9a-z]{1,30}$/i',$id)){
				foreach($arr as $arrkey=>$arrv){
				    if(!in_array($id,array('preview','uimage','notify_data','content','config','group_power','description','body','job_desc','eligible','other','code','intro','doc','traffic','media','packages','booth','participate','expinfo','eduinfo','skillinfo','projectinfo','token','deviceToken','xcxCode','rsaSign','sy_publicKey','sy_privateKey','sign','fund_bill_list','buyer_logon_id'))){
