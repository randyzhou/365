...
class com_controller extends wap_controller{
		...
		if($_POST['submit']){
			...
			if($data['msg'] == ''){
			    ...
			    if (!empty($cert) && $cert['ctime']) {
			        ...
			        $err   =   $comM -> upCertInfo(array('id'=>intval($cert['id']), 'uid' => $uid), $upData, array('yyzz' => '1', 'usertype' => $usertype, 'com_name'=>trim($_POST['name'])));
	...
	function sysnews_action(){
		...
		$jobnum			=	$MsgM->getMsgNum(array('job_uid'=>$this->uid,'reply'=>''));
	...
	function resumecolumn_action(){
	    ...
	    //被浏览的职位数
	    $lookjobnum		=	$jobM -> getLookJobNum(array('com_id'=>$this->uid,'com_status'=>'0'));
    ...
    function server_action(){
        ...
        if ($statis['vip_etime'] < strtotime('tomorrow') && $statis['vip_etime'] != 0 ) {   //  过期会员
	...
	function toresume_action(){
		$JobM		=   $this -> MODEL('job');
        $id			=	$_POST['id'];
		$nid		=	$JobM->updSqJob(array('id'=>$id,'com_id'=>$this->uid),array('is_browse'=>2));
	}
