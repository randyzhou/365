...
class index_controller extends wap_controller{
	...
	function index_action()
	{
		...
		$expect    =  $ResumeM->getExpect(array('uid'=>$this->uid,'defaults'=>1),array('field'=>'integrity,id,lastupdate'));
		...
		if($this->config['resume_sx']==1){//登录自动简历刷新,在后台配置
			...
			if($user['def_job']){
			    ...
			    $ResumeM -> upInfo(array('id'=>$user['def_job'],'uid'=>$this->uid),array('eData'=>array('lastupdate'=>time())));
	...
	function inviteset_action(){
		...
		$return	=	$JobM -> setYqms(array('id'=>$id,'browse'=>$browse,'uid'=>$this->uid,'username'=>$this->username));
	...
	function addresume_action(){
		...
		$return		=	$resumeM	->	addResumePage(array('uid'=>$this->uid,'needcache'=>1,'from'=>'wap'));
	...
	function saveresumeson_action(){
		...
		if($_POST['submit']){
			...
			if($_POST['eid']>0){
				...
				if($nid){
					$data['msg']	=	'保存成功！';
					$ResumeM->upExpectHeight(array('eid'=>(int)$_POST['eid']));
				}else{
					$data['msg']	=	'保存失败！';
				}
   ...
   */
    ...
    {	
		...
		if($_GET['eid']){
		    $resumeM  =  $this->MODEL('resume');
		    $return   =  $resumeM->getInfo(array('uid'=>$this->uid,'eid'=>intval($_GET['eid']),'needCache'=>1));
	...
	function resume_action(){
		...
		if ($_GET['eid']){
			$expect	=	$ResumeM->getExpect(array('id'=>(int)$_GET['eid'],'uid'=>$this->uid));
		}
		if(!$expect['id']){
			$expect	=	$ResumeM->getExpect(array('default'=>1,'uid'=>$this->uid));
		
			if (!$expect){

				$rwhere['uid']		=	$this->uid;
				$rwhere['orderby']	=	array('lastupdate,desc');
				$expect				=	$ResumeM->getExpect($rwhere);
			}		
		}
		...
		$resume	=	$ResumeM->getUserResumeInfo(array('eid'=>$expect['id']));
		$this->yunset("resume",$resume);
	...
	function resumeset_action(){
		...
		if($_GET['del']){
			...
			$heightwhere['height_status']				=					array('>',0);
			
			$rows										=					$ResumeM->getExpect($heightwhere);
			
			if(!empty($rows)&&$id!=$rows['id']){

				$this->waplayer_msg('一个人只能申请一份优质简历！');
				...
				if($gdeu!=1){
							...
							$this->waplayer_msg('学历本科以上才可以申请优质简历！');
					
				}
				
				$workwhere['eid']							=				(int)$_GET['height'];
				
				$workwhere['uid']							=				$this->uid;

				$wklist										=				$ResumeM->getResumeWorks($workwhere);
				if(is_array($wklist)){
				...
				if(!($whour>24 || $worknum>3)){
				    if ($row['whour']<24){
						...
						$this->waplayer_msg('工作经历二年以上才可以申请优质简历！');
					
					}elseif ($worknum<4){
					 
						$this->waplayer_msg('工作经历三项以上才可以申请优质简历！');
				 ...
				 if($this->config['user_height_resume']=='2'){
						...
						$resumeexpectwhere['id']			=			$id;

						$resumeexpectwhere['uid']			=			$uid;
						
						$nid								 =			$ResumeM->upInfo($resumeexpectwhere,array('eData'=>$resumeexpectData));
	...
	function binding_action(){
		...
		if($_POST['moblie']){
			...
			$return=$CompanyM -> upCertInfo(array('uid'=> $this->uid, 'check2' => $_POST['code']), array('status'=>'1'), $data);
			echo $return;die;
		}
		// 解除绑定
		if($_GET['type']){
    ...
    */
	...
	function changelist_action(){
		...
		$this->getStatis();
		$this->yunset('backurl',$backurl);
		$this->yunset('headertitle',"赏金转换".$this->config['integral_pricename']."明细");
