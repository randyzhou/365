...
class index_controller extends adminCommon{
    ...
    function index_action($set = ''){
        ...
        $urlarray     =  explode('/', $this->config['sy_weburl']);
        
        if ($http_host != $urlarray[2]) {
            
            $https  =  $_SERVER['HTTPS'];
            $http   =  $https == 'on' ? 'https://' : 'http://';
            
            if ($urlarray[2] != 'localhost' && $urlarray[2] != '127.0.0.1') {
                header('Location: ' . $http . $urlarray[2] . $request_url);
            }
        }*/
            ...
            if (strpos($this -> config['code_web'], '后台登录') !== false) {
                ...
                if ($this -> config['code_kind'] == 3) {
                    ...
                    if (! dxauthcode($this->config)) {
                        $this -> ACT_layer_msg('请滑动按钮进行验证！', 8, 'index.php');
                    } else {
                        $this -> admin_get_user_login($_POST['username'], $_POST['password'], 'index.php');
                    }
