...
class admin_company_controller extends adminCommon{
	...
	function uprating_action(){
        ...
        if(empty($rid)){
            ...
            $sonList     =   $this->obj->select_all('company_account', array('comid' => $uid, 'status' => 1), '`uid`');
            unset($_POST['ratuid']);
            unset($_POST['pytoken']);
            
            if(intval($_POST['addday']) > 0){
                
                if(intval($_POST['oldetime'])>time()){
            ...
            if (is_array($sonList) && !empty($sonList)) {
                ...
                if($value['rating_type'] == 2){ //  时间会员，变更套餐数据（每日上限）
                    ...
                    if ($statis['rating'] == $value['rating'] && ($statis['vip_etime'] >= strtotime('today') || $statis['vip_etime'] == 0)) {    
            ...
            if($statis['rating'] != $_POST['rating'] || ($statis['rating'] == $_POST['rating'] && $statis['vip_etime'] < strtotime(date('Y-m-d')) && $statis['vip_etime'] !=0 && (int)$_POST['addday'] == 0)){
				...
				$content	=	'企业：'.$company['name'].' 会员等级修改为'.$rating_info['name'].'！';
			...
			if($_POST['hotjob']){
				...
				if(!empty($hotjob)){
					...
					$arr  =   $companyM -> upHotJob($uid, $hotData);  
				}else{
					$hotData['uid']	=	$uid;
					$hotData['username']	=	$company['name'];
					$hotData['did']	=	$company['did'];
					$arr  =   $companyM -> addHotJob($hotData);
				}
			}else{
				if($company['rec']==1){
					$nid	=	$companyM -> delHotJob($uid);
				}
			}
