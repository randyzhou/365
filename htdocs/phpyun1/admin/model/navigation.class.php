...
class navigation_controller extends adminCommon{
	function index_action(){
		...
		$search_list[]=array("param"=>"nid","name"=>'导航位置',"value"=>$nclass);
	...
	function save_action(){
		...
		if($_POST['update']){
			...
			if (isset($nbid['msg'])){
			    $this->ACT_layer_msg($nbid['msg'],8,"index.php?m=navigation");
			}
			$this->cache_action();
			$lasturl	=	str_replace("&amp;","&",$_POST['lasturl']);
			isset($nbid)?$this->ACT_layer_msg( "网站导航(ID:".$_POST['id'].")更新成功！",9,"index.php?m=navigation",2,1):$this->ACT_layer_msg( "更新失败！",8,"index.php?m=navigation");
		}
		//添加
	    if($_POST['add']){
