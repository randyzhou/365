...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
		...
		this.config = {
			...
			template: function template(id, item) {
				...
				return item.name;//Number(item.upclass) ? (Number(item.upclass)==1 ? "┗"+item.name : "┗"+item.name) : item.name;
			},
			showCount: 0,
			isCreate: false,
			placeholder: TIPS,
	...
	Common.prototype.init = function (target) {
		...
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			...
			$('div.' + PNAME + '[FS_ID="' + id + '"] .' + INPUT).click(function (e) {
				_this2.search(id, e, fs.config.searchUrl);
			});
		});
	};
	...
	Common.prototype.search = function (id, e, searchUrl, call) {
		...
		var inputValue = input ? $.trim(input.value) : '';
		//过滤一下tips
		...
		if (searchUrl) {
			...
			if(fs.config.xmtype!='' && typeof(fs.config.xmtype) != "undefined"){
				var fsArr 	= 	[],
					thisclass 	= 	[],
					svalue 		= 	'',
					fs_parent	=	[],
				...
				if(inputValue!='' || fst.length<=0 || fst=='new Array()'){
					_this3.changeShow(div, true);
					if(inputValue!=''){
						if(fsn.length>0){
							var inputv 	=	inputValue.toLowerCase(),
								itemv 	=	'';
					...
					if(thisclass.length>0){
			        	...
			        	if(fsone.length>0){
			        		...
			        		for(var i=0;i<fsone.length;i++){
			        			...
			        			//江苏
			        			if(fst[fsone[i]] && fst[fsone[i]].length){
	...
	Common.prototype.renderData = function (id, dataArr, linkage, linkageWidth, isSearch, isReplace) {
		...
		if (isSearch) {
			...
			oldVal.forEach(function (item, index) {
				...
				dl.find('dd[lay-value]').each(function(oindex,oitem){
					svalstr = $(oitem).attr('lay-value');
					sval = Number(svalstr);
					toadd = false;
					if(fs.config.xmtype=='jobclass'){
						
						job_parent[sval] = typeof(job_parent[sval])!="undefined" ? job_parent[sval] : 0;
						if(jt[item.value] 
							&& (
								(jt[item.value].indexOf(sval)!=-1 || jt[item.value].indexOf(svalstr.toString())!=-1)
								|| (jt[item.value].indexOf(job_parent[sval])!=-1 || jt[item.value].indexOf(job_parent[sval].toString())!=-1)
								)
							){
							toadd = true;
						}
					}else if(fs.config.xmtype=='cityclass'){
						city_parent[sval] = typeof(city_parent[sval])!="undefined" ? city_parent[sval] : 0;
						if(ct[item.value] 
							&& (
								(ct[item.value].indexOf(sval)!=-1 || ct[item.value].indexOf(svalstr.toString())!=-1)
								|| (ct[item.value].indexOf(city_parent[sval])!=-1 || ct[item.value].indexOf(city_parent[sval].toString())!=-1)
								)
							){
							toadd = true;
						}
	...
	Common.prototype.createDD = function (id, item, clz) {
		...
		if(item.upclass==1){
			upclass='xm_upclass_two';
			classctr = 'xm_classctr_two';
		}else if(item.upclass==2){
			upclass='xm_upclass_three';
			classctr = 'xm_classctr_three';
		}else{
			classctr = 'xm_classctr_one';
		}
		
		return '<dd lay-value="' + item.value + '" class="'+ classctr + (item.disabled ? DISABLED : '') + ' ' + (clz ? clz : '') + '" ' + attr + '>\n\t\t\t\t\t<div class="xm-unselect xm-form-checkbox ' + (item.disabled ? DISABLED : '') + '"  style="margin-left: ' + (pid.length - 1) * 30 + 'px">\n\t\t\t\t\t\t<i class="' + CHECKBOX_YES + '"></i>\n\t\t\t\t\t\t<span name="' + name + '" class="'+upclass+'" >' + template +' '+upnameHtml+'</span>\n\t\t\t\t\t</div>\n\t\t\t\t</dd>';
	};
	...
	Common.prototype.one = function (target) {
		...
		$(target ? target : document).off('click', '.' + FORM_TITLE).on('click', '.' + FORM_TITLE, function (e) {
			...
			if (othis.is('.' + SANJIAO) || othis.is('.' + INPUT + '[readonly]')) {
				...
				_this11.search(id);//只有一级类别时直接下拉显示
	...
	Common.prototype.handlerLabel = function (id, dd, isAdd, oval, notOn) {
		...
		if(isAdd){//*jobclass *cityclass 检查 搜索出的下拉列表 中是否有属于 当前选中项 的 下级类别
			...
			for(var i=0;i<vals.length;i++){
				if(fs.config.xmtype=='jobclass'){
					if(jt[val.value]){
						jobparent = typeof(job_parent[vals[i].value])!="undefined" ? job_parent[vals[i].value] : 0;
						
						if((jt[val.value].indexOf(vals[i].value)!=-1 || jt[val.value].indexOf(vals[i].value.toString())!=-1)
							||(jt[val.value].indexOf(jobparent)!=-1 || jt[val.value].indexOf(jobparent.toString())!=-1)
							){
							//值操作
					...
					if(ct[val.value]){
						cityparent = typeof(city_parent[vals[i].value])!="undefined" ? city_parent[vals[i].value] : 0;
			...
			if(fs.config.xmtype=='jobclass'){
				...
				// })
				var jtwoarr = jt[val.value]&&jt[val.value].length>0 ? jt[val.value] : [],
					jthreearr = [];
				dd.parent().children('dd[lay-value]').each(function(index,item){
					var svalnum = Number($(item).attr('lay-value')),
						svalstr = $(item).attr('lay-value');
					if(jt[val.value]){//选中项含有次级分类 判断搜出项是否属于该选中项的次级 1,设为不可选状态和添加已选中状态样式;2,去掉输入框中已选的次级、值 
						
						if(jtwoarr.indexOf(svalnum)!=-1 || jtwoarr.indexOf(svalstr)!=-1){//查看是否属于选中项的次级城市
							$(item).addClass(DISABLED);
							$(item).children('div').addClass(DISABLED);
							$(item).addClass(THIS);
							//值操作
							...
							if(ji && ji.indexOf(val.value)!=-1){//首先判断选中项是否是一级城市
								for(var i=0;i<jtwoarr.length;i++){
									jthreearr = jt[jtwoarr[i]];//三级城市集合
									if(jthreearr && (jthreearr.indexOf(svalnum)!=-1 || jthreearr.indexOf(svalstr)!=-1)){
										//设为不可选状态
										$(item).addClass(DISABLED);
										$(item).children('div').addClass(DISABLED);
										$(item).addClass(THIS);
				...
				var ctwoarr = ct[val.value]&&ct[val.value].length>0 ? ct[val.value] : [],
					cthreearr = [];
				dd.parent().children('dd[lay-value]').each(function(index,item){
					var svalnum = Number($(item).attr('lay-value')),
						svalstr = $(item).attr('lay-value');
			...
			if(fs.config.xmtype=='jobclass'){
				...
				// 	var svalnum = Number($(item).attr('lay-value')),
				// 		svalstr = $(item).attr('lay-value');
				// 	if(jt[val.value] && (jt[val.value].indexOf(svalnum)!=-1 || jt[val.value].indexOf(svalstr)!=-1)){
				// 		$(item).removeClass(THIS);
				// 		$(item).removeClass(DISABLED);
				// 		$(item).children('div').removeClass(DISABLED);
				// 	}
				// })
				var remove	=	false,
					svalnum = 	0,
				...
				dd.parent().children('dd[lay-value]').each(function(index,item){
					remove	=	false;
					svalnum = 	Number($(item).attr('lay-value')),
					svalstr = 	$(item).attr('lay-value');
					if(jt[val.value]){
						if(jt[val.value].indexOf(svalnum)!=-1 || jt[val.value].indexOf(svalstr)!=-1){
							remove = true;
						}else if(ji.indexOf(val.value)!=-1 || ji.indexOf(val.value.toString())!=-1){//点中的是一级的话，还需要查看是否有跨级
							for(var i=0;i<jt[val.value].length;i++){
								if(jt[jt[val.value][i]] && (jt[jt[val.value][i]].indexOf(svalnum)!=-1 || jt[jt[val.value][i]].indexOf(svalstr)!=-1)){
									remove = true;
	...
	Common.prototype.delLabel = function (id, div, val) {
		...
		if(fs.values.length==0){
			...
			div.find('input').attr('placeholder','');
		}
	};
