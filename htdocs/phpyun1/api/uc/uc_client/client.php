...
function uc_fopen($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
        ...
        $fpflag = 0;
        if(!$fp = @fsockopen(($scheme == 'https' ? 'ssl' : $scheme).'://'.($scheme == 'https' ? $host : ($ip ? $ip : $host)), $port, $errno, $errstr, $timeout)) {
                $context = array(
                        'http' => array(
...
function uc_mail_queue($uids, $emails, $subject, $message, $frommail = '', $charset = 'utf-8', $htmlon = FALSE, $level = 1) {
