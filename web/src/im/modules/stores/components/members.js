...
export default class Members extends React.Component{
  ...
  showAll=()=>{
    history.replaceState("","","#/stores/members/"+this.props.id+"");
  }  
  ...
  joinStore=(storeid)=>{
    var url = Config.api+"member/joinstore?ownerid="+storeid;
    fetchApi({
      url
    },function(res){ 
      resultAlert(res);
      if(res.result == 3){
        ...
        socket.emit("joinroom", {type:"Store", roomid:"Store"+storeid});
      }
    }.bind(this));
  }

  leaveStore=(storeid)=>{
    alertMsg("确定退出吗？",1,function(){
      var url = Config.api+"member/leavestore";  
      fetchApi({
        url,
        method: 'POST',
        body: {ownerid: storeid}
      },function(res){   
        resultAlert(res);
        if(res.result == 3){   
            this.setState({
          ...
          socket.emit("leaveroom", {type:"Store", roomid:"Store"+storeid});
        }
      }.bind(this));
    }.bind(this));
  }
  ...
  receiveChildData=(obj)=>{
    this.setState({shop: obj.shop,su:obj.su,searchFlag:0});
  }
    ...
    return (
      <div ref="main" style={{width:"100%",height:"100%"}}>
        ...
        <tr style={{height:"43px"}}><td>  
          ...
          <tr>
            <td style={{width:"80px",textAlign:"center"}}>
              <span style={{marginRight:"5px",display:$distanceshow?'inline':'none'}} onTouchEnd={this.handleLocationSearch}>附近</span>
              <span style={{marginRight:"5px",display:$distanceshow?'none':'inline'}} onTouchEnd={this.showAll}>所有  </span>
            </td>          
            <td style={{textAlign:"center"}} onClick={this.showSearchDiv}>
              <div style={{borderRadius:"3px",width:"100%",height:"40px",lineHeight:"40px"}}>搜索</div>
            </td>
            <td style={{width:"80px",textAlign:"center"}}>
              <div style={{display:su == 2?"block":"none",width:"34px",borderRadius:"5px",fontSize:"10px",margin:"0 auto",backgroundColor:"burlywood",color:"white"}} onClick={this.joinStore.bind(this,this.state.shop.id)}>加入</div>                    
              <div style={{display:su == 1?"block":"none",color:"darkgreen"}} onClick={this.leaveStore.bind(this,this.state.shop.id)}>退出</div>  
            </td>
          </tr>
          </tbody></table>
        </td></tr>
        <tr style={{height:"40px"}}><td>
          <table style={{width:'100%',height:"30px",backgroundColor:'#fff'}}>
            <tbody>
              <tr>
        ...
        <tr><td style={{overflowY:"scroll",verticalAlign:"top"}}>  
            <Listall 
              apiurl={Config.api + "member/storemembers?ownerid="+this.props.id} 
              hashurl={"#/stores/members/"+this.props.id}
              backurl={"#/stores/members/"+this.props.id} 
              searchData={this.state.searchData}
              item={ListItem}
              searchFlag={this.state.searchFlag}
              receiveChildData={this.receiveChildData}
              params={{shop:this.state.shop,certified}}
            />
          </td></tr>
        </tbody></table>    
