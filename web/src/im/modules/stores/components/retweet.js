export default class Retweet extends React.Component{
  ...
  componentDidMount(){
    if(this.props.id){
      let url = Config.api + "retweet/view?retweetid="+this.props.id+"&typeid=5";
      fetchApi({
        url
      },function(res){             
        if(!isJson(res)){
  ...
  shopDetail=(event)=>{
    ...
    if(this.state.retweet && this.state.retweet.ownerid){
      ...
      backurl_store = "#/stores/retweet/"+this.props.id;
  ...
  productDetail=()=>{
    if(this.state.retweet && this.state.retweet.ownerid){
      backurl_store = "#/stores/retweet/"+this.props.id;
      touchEnd("#/stores/product/"+this.state.retweet.id);
    }
  }
  ...
  render(){
    ...
    var ListItems = !this.state.comments?null:this.state.comments.map(function (item,index) {
      return (
        <Comment key={item.id} item={item} backurl={"#/stores/retweet/"+$this.props.id}/>
      );
    }.bind(this));
    ...
    let $avatars = [];
    var avatar = this.state.retweet?this.state.retweet.avatar:'';
                          ...
                          <table style={{width:"100%"}}>
                              ...
                              <tr><td style={{padding:"5px",paddingTop:"0px",textAlign:"center"}}>
                                ...
                                <div style={{textAlign:"left",fontSize:"18px",fontWeight:"bold"}}>
                                  <span onTouchEnd={this.productDetail}>{this.state.retweet?this.state.retweet.title:''}</span>
                                  <span style={{color:'green',float:"right",marginRight:"10px"}}>{this.state.retweet.price}元</span>
                                </div>
                                <div style={{height:"10px"}}></div>
                                <div style={{textAlign:"left",color:"#999999"}}><span onTouchEnd={this.shopDetail} style={{color:"#406599"}}>{this.state.retweet?this.state.retweet.owner_name:''}</span> <span style={{marginLeft:"10px"}}>{this.state.retweet?this.state.retweet.datetime:''}</span></div>
                                <div style={{height:"10px"}}></div>
                                <div style={{textAlign:"left",fontSize:"16px",border:"1px solid rgb(230,230,230)",borderRadius:"2px",backgroundColor:"rgb(250,250,250)",color:"rgb(100,100,100)",fontSize:"14px",padding:"5px",paddingLeft:"2px"}}>
                                  <span style={{color:"#406599"}}>{this.state.retweet.retweet_username}</span>荐语：{this.state.retweet?this.state.retweet.retweet_content:''}
                                </div>
                                <div style={{height:"10px"}}></div>
